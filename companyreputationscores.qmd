...
```{r}
#import data set + custom ggplot theme
library(tidyverse)
...
#filter for companies that are in retail or tech and for 2022 data  
...
#calculate avg reputation score b/t popular companies in retail industry and tech industry
company_poll_2022 |> 
  filter(industry == "Retail" | industry == "Tech") |>
  group_by(industry) |> 
...
#create function to repeatedly shuffle data points 
...
Now that I have a permutation function, I will run it 500 times using the map function below. This will generate 500 simulated null data sets that can be used to show the distribution of permuted mean differences on a histogram, which is included below. In the histogram, the observed average difference in the original data set is shown with a brown line.

```{r}
#| fig-alt: "Histogram depicting the distribution of permuted mean differences between reputation scores of retail companies and reputation scores of technology companies, with the observed average difference displayed as a brown line."

#ensure data is reproducible
set.seed(50) 

#iterate function 500 times using map 
perm_stats <- 
  map(c(1:500), perm_data, data = company_poll_2022) |> 
  list_rbind()

#graph the distribution of permuted mean differences on a histogram 
perm_stats |> 
  ggplot(aes(x = perm_avg_diff)) + 
  geom_histogram(fill = "mistyrose2") + 
  geom_vline(aes(xintercept = obs_avg_diff), color = "mistyrose4") +
  labs(
    title = "Distribution of Permuted Mean Differences",
    subtitle = "Brown line: observed difference in means",
